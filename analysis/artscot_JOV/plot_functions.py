...
def spatially_bin_img(x, y, param, ax, ecc_bounds, pol_bounds, scotoma_info=[], **kwargs):
    ''' 
    TODO...    
    '''
    ...
    img_ecc_grid, img_pol_grid = dag_coord_conver(img_x_grid,img_y_grid,old2new="cart2pol")
    # img_ecc_grid = np.rot90(img_ecc_grid)
    # img_pol_grid = np.rot90(img_pol_grid)

    pol_ecc, pol_angle = dag_coord_conver(x,y,old2new="cart2pol")
    ...
    # img_matrix *=0
    # img_matrix[a] = 900
    # ax.imshow(img_x_grid, vmin=0)
    # sys.exit()
    
    im_col = ax.imshow(np.flipud(img_matrix), extent=[-max_ecc, max_ecc, -max_ecc, max_ecc])
    fig = plt.gcf()
    fig.colorbar(im_col, ax=ax)

    # if len(ecc_bounds)>0:
    #     add_scot_patches_and_bin_lines(ax, ecc_bounds, pol_bounds, scotoma_info=scotoma_info, incl_patches=incl_patches, incl_ticks=incl_ticks)
    
    ax.set_aspect('equal')


def basic_spatially_binned_param(x, y, param, ax, ecc_bounds=[], pol_bounds=[], scotoma_info=[], **kwargs):
    ''' 
    TODO...    
    '''
    ...
    scat_col = ax.scatter(x_bin, y_bin, c=p_bin, s=dot_sizes, alpha=dot_alpha)
    fig = plt.gcf()
    fig.colorbar(scat_col, ax=ax)
...
def independent_arrow_plot(x4bin, y4bin, old_x, old_y, new_x, new_y, ax, ecc_bounds, pol_bounds, scotoma_info=[], **kwargs):
    ''' 
    ...
    ecc4bin, pol4bin = dag_coord_conver(x4bin,y4bin,old2new="cart2pol") # convert coords into polar coords
    total_n_bins = (len(ecc_bounds)-1) * (len(pol_bounds)-1)
    ...
    ax.scatter(binned_old_x, binned_old_y, color=old_col, s=100, alpha=dot_alpha)
    ax.scatter(binned_new_x, binned_new_y, color=new_col, s=100, alpha=dot_alpha)
    ax.quiver(binned_old_x, binned_old_y, dx, dy, scale_units='xy', angles='xy', alpha=dot_alpha, color='r', scale=1)

    add_scot_patches_and_bin_lines(ax, ecc_bounds, pol_bounds, scotoma_info=scotoma_info, incl_patches=incl_patches)
...
def rapid_arrow(p1, p2):
    ...
    ecc4bin, pol4bin = dag_coord_conver(old_x,old_y,old2new="cart2pol") # convert coords into polar coords
    total_n_bins = (len(ecc_bounds)-1) * (len(pol_bounds)-1)
    ...
    ax.scatter(binned_old_x, binned_old_y, color=old_col, s=100, alpha=dot_alpha)
    ax.scatter(binned_new_x, binned_new_y, color=new_col, s=100, alpha=dot_alpha)
    ax.quiver(binned_old_x, binned_old_y, dx, dy, scale_units='xy', angles='xy', alpha=dot_alpha, scale=1)

    add_scot_patches_and_bin_lines(ax, ecc_bounds, pol_bounds)

    return


def basic_arrow_plot(old_x, old_y, new_x, new_y, ax, **kwargs):
    ''' 
    PLOT FUNCTION: Takes starting coords (x,y) and end coords (new_x, new_y) produces a plot, with arrows from old to new points 
    >> ax                              specify axes to plot on
    >> ecc_bounds, pol_bounds           if included, the points will be binned (with ref to old coords)
    ...
    if do_binning:
        ...
        old_ecc, old_ang = dag_coord_conver(old_x, old_y,old2new="cart2pol")
        total_n_bins = (len(ecc_bounds)-1) * (len(pol_bounds)-1)
    ...
    ax.scatter(old_x2plot, old_y2plot, color=old_col, s=100, alpha=dot_alpha)
    ax.scatter(new_x2plot, new_y2plot, color=new_col, s=100, alpha=dot_alpha)
    
    # Add the arrows 
    if not do_shift_arrows: # Arrows all the same color
        ax.quiver(old_x2plot, old_y2plot, dx, dy, scale_units='xy', angles='xy', alpha=dot_alpha, scale=1)
    ...
    if len(ecc_bounds)>0:
        add_scot_patches_and_bin_lines(ax, ecc_bounds, pol_bounds, scotoma_info=scotoma_info, incl_patches=incl_patches, patch_col=patch_col)

def add_scot_patches_and_bin_lines(ax, ecc_bounds=[], pol_bounds=[], scotoma_info=[], incl_patches=False, **kwargs):
    do_ecc_bin_lines = kwargs.get('do_ecc_bin_lines', True)
    patch_col = kwargs.get("patch_col", 'k')
    if ecc_bounds==[]:
        ecc_bounds = np.linspace(0, 5, 7)
        pol_bounds = np.linspace(0, 2*np.pi, 13)
    ...
    for i_pol in range(n_polar_lines):
        ...
        if not '360' in outer_txt:
            ax.plot((0, outer_x), (0, outer_y), color="k", alpha=0.3)
            if incl_ticks:
...
def d2_scot_plot(x, y, new_x, new_y, scotoma_info, edge_or_com='com',  **kwargs):
    ...
    if do_plot:
        if do_scatter:
            ax.scatter(d2_target_old, change_param, color=col, alpha=0.5)

        if do_line:
            ax.errorbar(d2_mean, change_mean, change_stdE, color=col, linewidth=2, alpha=0.9, label=line_label, marker="s")                
        ...
        # ax.legend()
        ax.plot(d2_range, (0, 0), color="k")

        ax.set_xlim(d2_range)
...
def binned_plot(X,Y, **kwargs):
    ...
    # [1] Scatter values...
    if do_scatter:
        ax.scatter(X,Y, color=scat_col, alpha=0.1)
...
def d2_scot_param_plot(x, y, d_param, scotoma_info, **kwargs):
    ...
    if do_scatter:
        ax.scatter(d2_target, d_param, color=scat_col, alpha=0.1)
    
    if do_bin_line:
        plot_bin_line(
            X=d2_target,Y=d_param,bin_using=d2_target, 
            ax=ax, n_bins=n_bins, line_col=line_col, line_label=line_label
            )
...
def update_ax_fontsize(ax, new_font_size):
...
def update_fig_fontsize(fig, new_font_size):
    ...
    for i_kid in fig_kids:
        if isinstance(i_kid, mpl.axes.Axes):
            update_ax_fontsize(i_kid, new_font_size)
...
def update_ax_dotsize(ax, new_dot_size):
    # print(type(ax))
...
def update_fig_dotsize(fig, new_dot_size):
    for i_kid in fig.get_children():
        # print(i_kid)
        if isinstance(i_kid, mpl.axes.Axes):
            update_ax_dotsize(i_kid, new_dot_size)
        elif isinstance(i_kid, mpl.figure.SubFigure):
