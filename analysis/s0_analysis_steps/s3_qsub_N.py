...
'''
********* S3 *********
Fit the different conditions with the normalization model
...
for sub in sub_list:
    ...
    for task in task_list:
        prf_job_name = f'G{sub}_{task}'            
        # job=f"qsub -q long.q@jupiter -pe smp {nr_jobs} -wd {this_dir} -N {prf_job_name} -o {prf_job_name}.txt"
        job="python"
        script_path = opj(os.path.dirname(__file__),'s3_N_fit.py')
        script_args = f"--sub {sub} --model norm --task {task} --constraints bgfs " +\
            f"--prf_out {prf_out} --roi_fit demo-100 --nr_jobs 10 " 
